...
export default function Square({ index, color, hover, chip, onSquareClick, onMouseMove, onMouseLeave, combinations }: SquareInterface) {
    ...
    return (
        <div className='square'>
            ...
            {chip &&
                <Image style={{ top: `${50}%`, left: `${50}%` }} src={`/cursors/${chip}.png`} alt="chip_icon" className='chip_icon' width={35} height={35}/>
            }
            {combinations.map((combination, i) => {
                return combination.chip  && <Image key={i} style={{ top: `${combination.top}%`, left: `${combination.left}%` }} src={`/cursors/${combination.chip}.png`} alt="chip_icon" className='chip_icon' width={35} height={35}/>
            })}
        </div>
    );
}
